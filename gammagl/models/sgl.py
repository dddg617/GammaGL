...
from gammagl.models.loss import EmbLoss
...
class SGL(tlx.nn.Module):
    ...
    def __init__(self, config, dataset):
        ...
        self.user_embedding = tlx.nn.Embedding(self.n_users, self.latent_dim)
        self.item_embedding = tlx.nn.Embedding(self.n_items, self.latent_dim)
    ...
    def random_graph_augment(self):
        ...
        row = tlx.gather(edge_index, tlx.convert_to_tensor([0]))
        col = tlx.gather(edge_index, tlx.convert_to_tensor([1]))
        
        edge_weight = norm_deg[row] * norm_deg[col]

        return edge_index.to(self.device), edge_weight.to(self.device)

    def forward(self, graph=None):
        all_embeddings = tlx.concat([self.user_embedding.weight, self.item_embedding.weight])
        ...
        embeddings_list = tlx.stack(embeddings_list, axis=1)
        embeddings_list = tlx.reduce_mean(embeddings_list, axis=1, keepdim=False)
        user_all_embeddings, item_all_embeddings = tlx.split(embeddings_list, [self.n_users, self.n_items], axis=0)
    ...
    def calc_bpr_loss(self, user_emd, item_emd, user_list, pos_item_list, neg_item_list):
        ...
        p_scores = tlx.reduce_sum(tlx.multiply(u_e, pi_e), axis=1)
        n_scores = tlx.reduce_sum(tlx.multiply(u_e, ni_e), axis=1)

        l1 = tlx.reduce_sum(-tlx.log_sigmoid(p_scores - n_scores))
    ...
    def calc_ssl_loss(self, user_list, pos_item_list, user_sub1, user_sub2, item_sub1, item_sub2):
        ...
        u_emd1 = tlx.l2_normalize(user_sub1[user_list], axis=1)
        u_emd2 = tlx.l2_normalize(user_sub2[user_list], axis=1)
        all_user2 = tlx.l2_normalize(user_sub2, axis=1)
        v1 = tlx.reduce_sum(u_emd1 * u_emd2, axis=1)
        v2 = tlx.matmul(u_emd1, tlx.transpose(all_user2, [1,0]))
        # v2 = u_emd1.matmul(all_user2.T)
        v1 = tlx.exp(v1 / self.ssl_tau)
        v2 = tlx.reduce_sum(tlx.exp(v2 / self.ssl_tau), axis=1)
        ssl_user = -tlx.reduce_sum(tlx.log(v1 / v2))

        i_emd1 = tlx.l2_normalize(item_sub1[pos_item_list], axis=1)
        i_emd2 = tlx.l2_normalize(item_sub2[pos_item_list], axis=1)
        all_item2 = tlx.l2_normalize(item_sub2, axis=1)
        v3 = tlx.reduce_sum(i_emd1 * i_emd2, axis=1)
        v4 = tlx.matmul(i_emd1, tlx.transpose(all_item2, [1,0]))
        # v4 = i_emd1.matmul(all_item2.T)
        v3 = tlx.exp(v3 / self.ssl_tau)
        v4 = tlx.reduce_sum(tlx.exp(v4 / self.ssl_tau), axis=1)
        ssl_item = -tlx.reduce_sum(tlx.log(v3 / v4))
    ...
    def predict(self, interaction):
        ...
        return tlx.reduce_sum(user * item, axis=1)
    ...
    def full_sort_predict(self, interaction):
        ...
        user = self.restore_user_e[interaction[self.USER_ID]]
        return tlx.matmul(user, tlx.transpose(self.restore_item_e, [1, 0]))