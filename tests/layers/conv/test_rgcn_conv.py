...
def test_rgcn_conv(cls, conf):
    ...
    idx1 = tlx.arange(4, dtype = tlx.int64)
    idx2 = tlx.arange(2, dtype = tlx.int64)
    ...
    if num_blocks is None:
        out2 = tlx.convert_to_numpy(conv((None, idx2), edge_index, edge_type))
        assert out2.shape == (2, 32)
        assert np.allclose(tlx.convert_to_numpy(conv((idx1, idx2), edge_index, edge_type)), out2)
